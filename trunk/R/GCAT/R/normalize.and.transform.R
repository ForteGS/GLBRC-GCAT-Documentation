...
########################################################################
#
#   This function adds a "log.OD" column to the "screen.data" slot of a well object with log-transformed data. the raw data is kept intact. 
#   It also checks to see if any of the raw OD values (before a certain timepoint) is below the blank OD. This can be disastrous for the log(OD) transform. 

#   input.well - an object of class well
#   use.log - gets added to the "use.log" slot of the well object. this will determine whether the log-transformed data 
#              or raw normalized data is returned using the function <data.from>.   
#  blank.value - user can enter a blank OD measurement for uninoculated wells. if NULL, defaults to the value of the first OD measurement of each well. 
#  start.index - which timepoint should be used as the first one after inoculation (defaults to the 2th one)
#  negative.OD.cutoff - if any ODs below the specified blank value are detected before this index timepoint, the entire well is discarded. 
