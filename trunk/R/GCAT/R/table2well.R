...
#   it calls on the function <gcat.reorganize.single.plate.data> to rearrange the table before creating the output object. 
#
#  It can be run on its own on a raw data file, or called by <load.xlsx> after that function reads an excel spreadsheet 
#   into R data frame objects.
 
#  Arguments : see <GCAT.main.R>

gcat.load.data = function(file.name = NULL, load.type = "csv", input.data = NULL, single.plate.ID = NULL, 
  plate.layout = NULL, 
  blank.value = NULL, single.plate = F, silent = T){
  ...
  ########################################################################
	...
	if(is.null(input.data)){
		# Either read from .csv or tab-delimited text. 
		if (load.type == "csv"){
			input.data = read.csv(file.name, stringsAsFactors=F, skip = input.skip.lines, fileEncoding='UTF-8')
		  
	   }
		else{
			input.data = read.table(file.name,stringsAsFactors=F, skip = input.skip.lines)
      }
  ...
  # Call <gcat.reorganize.single.plate.data> to arrange data from a single plate format file
	if(single.plate)
		input.data = gcat.reorganize.single.plate.data(input.data = input.data, 
  ...
  if(is.null(plate.layout)){  # If <plate.layout> is not provided, do not add strain information, and assume all wells are inoculated. 
	...
	for(plate in unique(dimnames(well.array)[[3]])){
        ...
        # If <plate.layout> refers to specific plates, then use those to find the correct row. Otherwise, generalize across all plates. 
...
gcat.reorganize.single.plate.data = function(input.data, blank.value = NULL, single.plate.ID = "Plate_1", silent=T){
...
gcat.start.times = function(input.well, time.format, start.index = 2) {  
